...
├── data/
│   ├── LF50K_Datasheet_Values.csv     # Cell datasheet values used by the model (Appendix A, Table A-1)
│   ├── LF50K_ECM_RC_Parameters.csv    # Thevenin R0/R1/C1/R2/C2 parameters (Appendix A, Table A-2)
│   ├── LF50K_OCV_SOC_Table.csv        # OCV-SOC lookup table (Appendix A, Table A-3)
│   └── LF50K_DriveCycle_Current.csv   # Shared current profile, 3600 pts @ 1 Hz (instructor-issued — add yours here)
└── docs/
...
3. Run the initialization script in your Command Window:
   ...
   Base.m
   ```
   This loads `Q_nominal`, `SOC_0`, the R0/R1/C1 (and R2/C2) parameters, the drive-cycle current as a `timeseries`, and the OCV‑SOC lookup table into the base workspace, reading from the CSV files in `data/`.
4. Open `Main.slx`.
